...
const LuxuryTravelDr: React.FC = () => {
  ...
  useEffect(() => {
    const isSubscribed = sessionStorage.getItem('luxurySubscribed') === 'true';
    const termsAccepted = sessionStorage.getItem('luxuryTermsAccepted') === 'true';
    ...
    <div className="relative flex w-screen flex-col items-center font-['Gilroy'] transition-all duration-150 ease-in-out">
      ...
      {isModalOpen && (
        <KarmaTravelDemoModal
          listId={process.env.NEXT_PUBLIC_LUXURY_TRAVEL_CONSTANT_CONTACT_LIST_ID}
          onTermsOpen={closeKarmaTravelDemoModal}
        />
      )}
      {isTermsOpen && <TermsModal onClose={closeTermsModal} buttonClass="bg-[#132979]" />}
      {isMeditationOpen && <MeditationModal onClose={closeJourneyModal} />}
      <p className="text-[#636363] font-['Urbanist'] text-[12px] mt-[0] mb-[50px] leading-[14px]">
