...
const Confidence = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const result = await fetchTracks('RenewMe', 'Boost Confidence', 1, 20);
  ...
  const chunkedData = [];
  for (let i = 0; i < confidencesData.length; i += 6) {
    chunkedData.push(confidencesData.slice(i, i + 6));
  }

  return (
    <div className="gap-[39px] mt-8">
      {/* Section Header */}
      <HomeTitle text="Confidence" link="user/renewme-home/confidence" carouselApi={api} />
